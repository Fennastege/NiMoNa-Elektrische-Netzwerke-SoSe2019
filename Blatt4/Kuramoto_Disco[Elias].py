...
Nt= 0.7#(Länge des Zeitintervalls)
h=0.002 #Schrittweite der Zeit
t0=0 #Startwert der Zeit

###50 Oszillatoren mit Cauchyverteilten Phasen und Eigenfrequenzen generiere
theta=np.random.random(N)*2*np.pi
omega=np.random.standard_cauchy(N) ###Die Frequenzen sind normalverteilt und konvergieren gegen die mittlere Frequenz.
###Da hier die mittlere Frequenz 0 ist, hört das System quasi auf zu schwingen, weshalb sich auf der mittlere Vektor nicht mehr
###dreht, denn die Phase 'ruht'. Würde man einen konstanten Wert hinzufügen, würde der Mittelwert weiterschwingen, also rotieren
...
def plot(phase,mitphas, ordnungs, index):  #diese Funktion in gekoppelt implementieren	
	if(index%1==0):
		arrow=plt.arrow(0,0, ordnungs*np.cos(mitphas), ordnungs*np.sin(mitphas),color = list(np.random.choice(range(256), size=3)/256),linewidth=5, head_width=0.05)
